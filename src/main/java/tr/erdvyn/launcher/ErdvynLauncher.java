...
public final class ErdvynLauncher {
    ...
    static final class LauncherCanvas extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener, KeyListener, ActionListener {
        ...
        private void paintHome(Graphics2D g) {
            ...
            int py=top+112;paintNavIcon(g,3,x+11,py-4,AMBER);terminalLabel(g,l("SİSTEM PROFİLİ","SYSTEM PROFILE"),x+30,py);terminalPanel(g,x,py+18,leftW,222);String packageData=packSummary.files()==0?l("ÖLÇÜLÜYOR","MEASURING"):String.format(Locale.ROOT,l("%d MOD / %d DOSYA / %s","%d MODS / %d FILES / %s"),packSummary.mods(),packSummary.files(),formatBytes(packSummary.bytes()));String[][] rows={{l("MC SÜRÜMÜ","MC VERSION"),LauncherPaths.GAME_VERSION},{l("YÜKLEYİCİ","LOADER"),"NEOFORGE 21.1.243"},{l("PAKET VERİSİ","PACK DATA"),packageData},{l("DOSYA DURUMU","FILE STATUS"),packStatus.isBlank()?l("DENETİM BEKLİYOR","AUDIT PENDING"):l("DENETLENDİ","AUDITED")},{l("KURULUM","INSTANCE"),l("THE-FRONTIER / YÖNETİLEN","THE-FRONTIER / MANAGED")}};for(int i=0;i<rows.length;i++){int ry=py+55+i*32;g.setFont(font(13,Font.PLAIN));g.setColor(MUTED);g.drawString(rows[i][0],x+18,ry);g.setColor(PAPER);g.drawString(rows[i][1],x+154,ry);g.setColor(new Color(255,145,42,28));g.drawLine(x+14,ry+9,x+leftW-14,ry+9);}
            int buttonY=py+270;playBounds.setBounds(x,buttonY,Math.min(280,leftW),54);String playText=gameLaunching?l("BAŞLATILIYOR","STARTING"):accountSession==null?l("GİRİŞ YAP VE OYNA","SIGN IN & PLAY"):t("play");terminalButton(g,playBounds,hoverPlay,"[ "+upper(playText)+" ]");if(leftW>=460){g.setFont(font(12,Font.PLAIN));g.setColor(MUTED);g.drawString(l("BAŞLATMA HATTI 01","LAUNCH BUS 01"),x+300,buttonY+22);g.setColor(AMBER);g.drawString(accountSession==null?l("HESAP GEREKLİ","ACCOUNT REQUIRED"):l("HAZIR","READY"),x+300,buttonY+44);g.setColor(LINE);g.drawLine(x+405,buttonY+39,x+leftW-8,buttonY+39);}paintClientDiagnostics(g,x,buttonY+76,leftW,getHeight()-buttonY-104);updateBounds.setBounds(0,0,0,0);
        ...
        private void paintNews(Graphics2D g) {
            ...
            if(!newsNotice.isBlank()){g.setFont(font(11,Font.PLAIN));g.setColor(MUTED);g.drawString(localized(newsNotice),x+18,panelY+panelH-14);}
        }
        ...
        private void paintArticleOverlay(Graphics2D g){
            ...
            g.setFont(font(11,Font.PLAIN));g.setColor(AMBER);g.drawString(l("DUYURU / ","DISPATCH / ")+upper(t("news")),x+22,y+31);
            g.setColor(LINE);g.drawLine(x+12,y+44,x+boxW-12,y+44);
            g.setFont(font(23,Font.PLAIN));g.setColor(PAPER);String title=upper(post.title());g.drawString(title.length()>42?title.substring(0,39)+"...":title,x+22,y+82);
            ...
            g.setFont(font(10,Font.PLAIN));g.setColor(MUTED);g.drawString(l("TARİH ","DATE ")+formatNewsDate(post.publishedAt())+"   /   ID "+String.format(Locale.ROOT,"%04d",post.id()),x+22,y+274);
        ...
        private void paintWorldMap(Graphics2D g){
            ...
            g.setFont(font(11,Font.PLAIN));localSurvey.setTurkish(language==Language.TR);localSurvey.paint(g,x,y+72,getWidth()-x-32,getHeight()-y-106);
        }
        ...
        private void paintBootOverlay(Graphics2D g){
            int w=getWidth(),h=getHeight();g.setColor(new Color(0,0,0,208));g.fillRect(0,0,w,h);int bw=Math.min(790,w-110),bh=Math.min(540,h-88),x=(w-bw)/2,y=(h-bh)/2;terminalPanel(g,x,y,bw,bh);String[] logs=launcherBootMode?LAUNCHER_BOOT_LOGS:BOOT_LOGS;g.setFont(font(15,Font.PLAIN));g.setColor(PAPER);g.drawString(launcherBootMode?l("ERDVYN TERMİNALİ AÇILIYOR","ERDVYN TERMINAL STARTUP"):l("ERDVYN BAŞLATMA DİZİSİ","ERDVYN BOOT SEQUENCE"),x+22,y+34);g.setFont(font(11,Font.PLAIN));g.setColor(MUTED);g.drawString(launcherBootMode?l("DÜĞÜM: YEREL KONSOL / CRT: KEHRİBAR / HAT: UI-01","NODE: LOCAL-CONSOLE / CRT: AMBER / BUS: UI-01"):l("DÜĞÜM: ERDVYN-FRONTIER / ORTAM: JAVA 21","NODE: ERDVYN-FRONTIER / RUNTIME: JAVA 21"),x+22,y+55);g.setColor(LINE);g.drawLine(x+14,y+68,x+bw-14,y+68);
        ...
        private void onHubEvent(HubEvent event){SwingUtilities.invokeLater(()->{if(event.kind.equals("chat"))chatLines.add(new ChatLine(event.author,event.value,LocalTime.now()));else if(event.kind.equals("players")){onlinePlayers.clear();if(!event.value.isBlank())onlinePlayers.addAll(Arrays.stream(event.value.split(",")).map(String::strip).filter(s->!s.isEmpty()).toList());}else if(event.kind.equals("status"))chatLines.add(ChatLine.system("SYSTEM",event.value,LocalTime.now()));repaint();});}
